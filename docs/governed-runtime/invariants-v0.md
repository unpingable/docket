...
| 6 | No success or failure is minted from indeterminacy. | proved | GTK `fabricated_success_after_ambiguity_is_refused`, `unresolved_is_not_committed` |
| 7 | No automatic retry follows ambiguous dispatch. | proved | corollary of 6 |
| 8 | Recovery facts bind the exact attempt and dispatch. | proved | GTK `reconciliation_is_recovery_sourced` + `reconcile` typed mismatch refusals |
...
| 28 | `DispatchRefused` remains distinct from later `ProvenNotCommitted`. | proved | GTK type-level trichotomy; `recoveredNotCommitted` distinct constructor (names are this runtime's vocabulary) |
